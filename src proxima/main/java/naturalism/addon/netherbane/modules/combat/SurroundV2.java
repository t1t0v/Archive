...
public class SurroundV2 extends Module {
    public SurroundV2(){
        super(NetherBane.COMBATPLUS, "surroundV2", "-");
        ...
        if (antiBedAura.get()){
            ...
            for (int i = 0; i < getSurrDesign().toArray().length; i++) {
                ...
                if (BlockUtil.getBlock(bb) instanceof BedBlock){
                    ...
                    mc.interactionManager.interactBlock(mc.player, mc.world, Hand.OFF_HAND, new BlockHitResult(mc.player.getPos(), Direction.UP, bb, false));
