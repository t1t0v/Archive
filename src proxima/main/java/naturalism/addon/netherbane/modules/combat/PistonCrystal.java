...
public class PistonCrystal extends Module {
    public PistonCrystal(){
        super(NetherBane.COMBATPLUS, "piston-crystal", "-");
    }
            ...
            if (!crystal.getBlockPos().equals(crystalPos)) {
                if (BlockUtil.getBlock(pistonPos) == Blocks.PISTON && mc.world.getBlockState(crystalPos).isAir()){
                    if (crystalTimer <= 0){
                        ...
                        mc.interactionManager.interactBlock(mc.player, mc.world, mc.player.getOffHandStack().getItem() == Items.END_CRYSTAL ? Hand.OFF_HAND : Hand.MAIN_HAND, new BlockHitResult(mc.player.getPos(), Direction.DOWN, crystalPos.down(), true));
                        crystalTimer = crystalDelay.get();
                    }else crystalTimer--;
                }
            }
                ...
                if (redstoneMode.get() == RedstoneMode.Button){
                    if (clickTimer <= 0){
                        if (BlockUtil.buttons.contains(BlockUtil.getBlock(upPos))){
                            mc.interactionManager.interactBlock(mc.player, mc.world, clickHandMode.get(), new BlockHitResult(mc.player.getPos(), Direction.DOWN, upPos, true));
                        }
                        if (BlockUtil.buttons.contains(BlockUtil.getBlock(downPos))){
                            mc.interactionManager.interactBlock(mc.player, mc.world, clickHandMode.get(), new BlockHitResult(mc.player.getPos(), Direction.DOWN, downPos, true));
                        }
                        if (BlockUtil.buttons.contains(BlockUtil.getBlock(thirdPos))){
                            mc.interactionManager.interactBlock(mc.player, mc.world, clickHandMode.get(), new BlockHitResult(mc.player.getPos(), Direction.DOWN, thirdPos, true));
                        }
                        if (BlockUtil.buttons.contains(BlockUtil.getBlock(nearPiston1))){
                            mc.interactionManager.interactBlock(mc.player, mc.world, clickHandMode.get(), new BlockHitResult(mc.player.getPos(), Direction.DOWN, nearPiston1, true));
                        }
                        if (BlockUtil.buttons.contains(BlockUtil.getBlock(nearPiston2))){
                            mc.interactionManager.interactBlock(mc.player, mc.world, clickHandMode.get(), new BlockHitResult(mc.player.getPos(), Direction.DOWN, nearPiston2, true));
                        }
                        clickTimer = buttonClickDelay.get();
                    }else clickTimer--;
                }
    ...
    private boolean checkPattern(boolean redstoneBlock, boolean torch, boolean button , BlockPos crystalPos, BlockPos pistonPos, BlockPos thirdPos, BlockPos upPos, BlockPos downPos, BlockPos nearPiston1, BlockPos nearPiston2){
        if ((BlockUtil.isAir(crystalPos.down()) || BlockUtil.getBlock(crystalPos.down()) == Blocks.OBSIDIAN || BlockUtil.getBlock(crystalPos.down()) == Blocks.BEDROCK) && (BlockUtil.isAir(crystalPos) || BlockUtil.getBlock(crystalPos) == Blocks.PISTON_HEAD || BlockUtil.getBlock(crystalPos) == Blocks.MOVING_PISTON) && (BlockUtil.isAir(pistonPos) || BlockUtil.getBlock(pistonPos) == Blocks.PISTON || BlockUtil.getBlock(pistonPos) == Blocks.MOVING_PISTON)){
            if (redstoneBlock){
                if (BlockUtil.isAir(thirdPos) || BlockUtil.getBlock(thirdPos) == Blocks.REDSTONE_BLOCK){
                    return true;
                }
            }else if (torch){
                if (BlockUtil.isAir(nearPiston1) || BlockUtil.getBlock(nearPiston1) == Blocks.REDSTONE_TORCH || BlockUtil.isAir(thirdPos) || BlockUtil.getBlock(thirdPos) == Blocks.REDSTONE_TORCH || BlockUtil.isAir(nearPiston2) || BlockUtil.getBlock(nearPiston2) == Blocks.REDSTONE_TORCH){
                    return true;
                }
            }else if (button){
                if (BlockUtil.isAir(upPos) || BlockUtil.getBlock(upPos) == Blocks.STONE_BUTTON || BlockUtil.isAir(downPos) || BlockUtil.getBlock(downPos) == Blocks.STONE_BUTTON || BlockUtil.isAir(thirdPos) || BlockUtil.getBlock(thirdPos) == Blocks.STONE_BUTTON || BlockUtil.isAir(nearPiston1) || BlockUtil.getBlock(nearPiston1) == Blocks.STONE_BUTTON || BlockUtil.isAir(nearPiston2) || BlockUtil.getBlock(nearPiston2) == Blocks.STONE_BUTTON){
                    return true;
                }
            }
        }
    ...
    public static class RenderBlock {
        ...
        public int ticks;
        private MinecraftClient mc = MeteorClient.mc;
