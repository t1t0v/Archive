...
public class AutoBedCrafter extends Module {
    public AutoBedCrafter(){
        super(NetherBane.COMBATPLUS, "bed-craft", "AutoBedCraft by GhostTypes. Thanks");
    ...
    private final Setting<Boolean> openBook = sgAuto.add(new BoolSetting.Builder().name("open-book").description("Automatically place/search for and open crafting tables when you're out of beds.").defaultValue(true).build());
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (automatic.get() && needsRefill() && canRefill(true) && !isOutOfMaterial() && !(mc.player.currentScreenHandler instanceof CraftingScreenHandler)) {
            FindItemResult craftTable = ItemUtil.findCraftTable();
            if (!craftTable.found()) {
                toggle();
        ...
        if (mc.player.currentScreenHandler instanceof CraftingScreenHandler) {
            ...
            if (openBook.get()){
                if (!mc.player.getRecipeBook().isGuiOpen(RecipeBookCategory.CRAFTING)){
    ...
    private void placeCraftingTable(FindItemResult craftTable) {
        List<BlockPos> nearbyBlocks = BlockUtil.getSphere(mc.player.getBlockPos(), radius.get(), radius.get());
        for (BlockPos block : nearbyBlocks) {
            if (BlockUtil.getBlock(block) == Blocks.AIR) {
    ...
    private BlockPos findCraftingTable() {
        List<BlockPos> nearbyBlocks = BlockUtil.getSphere(mc.player.getBlockPos(), radius.get(), radius.get());
        for (BlockPos block : nearbyBlocks) if (BlockUtil.getBlock(block) == Blocks.CRAFTING_TABLE) return block;
        return null;
    }
    ...
    private boolean canRefill(boolean checkSlots) {
        if (!autoWhileMoving.get() && PlayerUtil.isPlayerMoving()) return false;
        if (autoOnlyHole.get() && !EntityUtil.isInHole(true ,mc.player)) return false;
    ...
    private boolean isOutOfMaterial() {
        FindItemResult wool = InvUtils.find(itemStack -> ItemUtil.wools.contains(itemStack.getItem()));
        FindItemResult plank = InvUtils.find(itemStack -> ItemUtil.planks.contains(itemStack.getItem()));
        FindItemResult craftTable = ItemUtil.findCraftTable();
