...
public class Offhand extends Module {
    public Offhand(){
        super(NetherBane.COMBATPLUS, "offhandV2", "Refill totems in the offhand slot.");
    ...
    private boolean isThereDangerousBlock(){
        ...
        BlockUtil.getSphere(mc.player.getBlockPos(), range.get(), range.get()).stream().filter(blockPos -> (!mc.world.getDimension().isBedWorking() && BlockUtil.getBlock(blockPos) instanceof BedBlock) || (!mc.world.getDimension().isRespawnAnchorWorking() && BlockUtil.getBlock(blockPos) == Blocks.RESPAWN_ANCHOR)).forEach(blockPos -> {
