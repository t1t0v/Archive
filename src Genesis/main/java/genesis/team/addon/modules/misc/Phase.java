...
public class Phase extends Module {
    ...
    public Phase() {
        super(Genesis.Misc, "phase", "Lets you walk through blocks.");
    }
    ...
    public void onActivate() {
        ...
        prevX = EntityInfo.X(mc.player);
        prevY = EntityInfo.Y(mc.player);
        prevZ = EntityInfo.Z(mc.player);
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        double pX = EntityInfo.X(mc.player);
        double pY = EntityInfo.Y(mc.player);
        double pZ = EntityInfo.Z(mc.player);

        if (EntityInfo.notNull(mc.player))
        ...
        if (mode.get() == Mode.Normal) {
            if (speed <= 1 || !BlockInfo.isAir(EntityInfo.getBlockPos(mc.player))) {
                if (mc.options.forwardKey.isPressed()) {
    ...
    private void onCollisionShape(CollisionShapeEvent event) {
        ...
        if (event.pos.getY() < EntityInfo.Y(mc.player)) {
