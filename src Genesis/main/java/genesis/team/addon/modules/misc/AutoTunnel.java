...
public class AutoTunnel extends Module {
    public AutoTunnel(){
        super(Genesis.Misc, "auto-tunnel", "-");
    }
    ...
    private final ArrayList<BlockPos> blockPoses = new ArrayList<>();
    ...
    private void onTick(TickEvent.Pre event){
        ...
        if (!mc.world.getBlockState(playerPos.down()).isFullCube(mc.world, playerPos.down())) {
    ...
    private boolean isTunnel(BlockPos b1, BlockPos b2){
        return !BlockUtil.isAir(b1.down()) && !BlockUtil.isAir(b2.up()) && ((!BlockUtil.isAir(b1.east()) && !BlockUtil.isAir(b1.west()) && !BlockUtil.isAir(b2.east()) && !BlockUtil.isAir(b2.west())) || (!BlockUtil.isAir(b1.south()) && !BlockUtil.isAir(b1.north()) && !BlockUtil.isAir(b2.south()) && !BlockUtil.isAir(b2.north())));
    }
    ...
    private boolean doBreak(BlockPos b1, BlockPos b2){
        if (breakScaffold.get()){
            ...
            return BlockUtil.isAir(b1) && BlockUtil.isAir(b2);
        }else return true;
    }
