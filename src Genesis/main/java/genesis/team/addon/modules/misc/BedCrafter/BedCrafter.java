...
package genesis.team.addon.modules.misc.BedCrafter;
...
public class BedCrafter extends Module {
    ...
    public BedCrafter() {
        super(Genesis.Misc, "bed-crafter", "Automatically craft beds.");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (mc.player.currentScreenHandler instanceof CraftingScreenHandler currentScreenHandler) {
            ...
            if (CraftUtils.isInventoryFull()) closeCraftingTable();
        }
    }

    private void placeCraftingTable(FindItemResult craftTable) {
        List<BlockPos> nearbyBlocks = BlockInfo.getSphere(mc.player.getBlockPos(), radius.get(), radius.get());
        for (BlockPos block : nearbyBlocks) {
            if (BlockInfo.getBlock(block) == Blocks.AIR && BlockUtils.canPlace(block, true)) {
    ...
    private BlockPos findCraftingTable() {
        List<BlockPos> nearbyBlocks = BlockInfo.getSphere(mc.player.getBlockPos(), radius.get(), radius.get());
        for (BlockPos block : nearbyBlocks) if (BlockInfo.getBlock(block) == Blocks.CRAFTING_TABLE) return block;
        return null;
    }

    private void openCraftingTable(BlockPos tablePos) {
        Vec3d tableVec = Vec3dInfo.closestVec3d(tablePos);
    ...
    private boolean canRefill(boolean checkSlots) {
        if (!autoWhileMoving.get() && EntityInfo.isMoving(mc.player)) return false;
        if (autoOnlyHole.get() && !EntityInfo.isInHole(mc.player)) return false;
        if (autoOnlyGround.get() && !mc.player.isOnGround()) return false;
        if (CraftUtils.isInventoryFull()) return false;
        if (checkSlots) if (CraftUtils.getEmptySlots() < emptySlotsNeeded.get()) return false;
