...
public class NotifierPlus extends Module {
    ...
    private final Setting<Boolean> sendNotifications = sgVisualRange.add(new BoolSetting.Builder()
        .name("notifications")
    ...
    private final Setting<StashFinder.Mode> notificationMode = sgVisualRange.add(new EnumSetting.Builder<StashFinder.Mode>()
    ...
    public NotifierPlus() {
        super(Genesis.Info, "notifier", "indra modules");
    }
    ...
    private void onEntityAdded(EntityAddedEvent event) {
        ...
        if (event.entity instanceof PlayerEntity) {
            if ((!visualRangeIgnoreFriends.get() || !Friends.get().isFriend(((PlayerEntity) event.entity))) && (!visualRangeIgnoreFakes.get() || !(event.entity instanceof FakePlayerEntity))) {
                ChatUtils.sendMsg(event.entity.getId() + 100, Formatting.GRAY, "(highlight)%s(default) Терпила детектед", event.entity.getEntityName());
            }
        }
        else {
    ...
    private void onEntityRemoved(EntityRemovedEvent event) {
        ...
        if (event.entity instanceof PlayerEntity) {
            if ((!visualRangeIgnoreFriends.get() || !Friends.get().isFriend(((PlayerEntity) event.entity))) && (!visualRangeIgnoreFakes.get() || !(event.entity instanceof FakePlayerEntity))) {
                ChatUtils.sendMsg(event.entity.getId() + 100, Formatting.GRAY, "(highlight)%s(default) съебался за кентами", event.entity.getEntityName());
            }
        } else {
            MutableText text = Text.literal(event.entity.getType().getName().getString()).formatted(Formatting.WHITE);
    ...
    private void onReceivePacket(PacketEvent.Receive event) {
        ...
        synchronized (totemPopMap) {
            ...
            ChatUtils.sendMsg(getChatId(entity), Formatting.GRAY, "(highlight)%s (default)проебал (highlight)%d (default)%s.", entity.getEntityName(), pops, pops == 1 ? "totem" : "totems");
        }
    }
        ...
        synchronized (totemPopMap) {
            for (PlayerEntity player : mc.world.getPlayers()) {
                ...
                if (player.deathTime > 0 || player.getHealth() <= 0) {
                    ...
                    ChatUtils.sendMsg(getChatId(player), Formatting.GRAY, "(highlight)%s (default)умер после (highlight)%d (default)%s.", player.getEntityName(), pops, pops == 1 ? "totem" : "totems");
