...
public class SilentPickaxe extends Module {
    public SilentPickaxe(){
        super(Genesis.Px, "silent-pickaxe-proxima", "");
    }
    ...
    private final MyBlock block = new MyBlock();
    ...
    public boolean isMiningBlock(BlockPos pos) {
        if (block.blockPos != null){
            return block.blockPos.equals(pos);
        }
        return false;
    }

    @EventHandler
    private void onUpdateBlock(BlockUpdateEvent event){
        if (block.blockPos != null){
            if (event.pos.equals(block.blockPos) && swapped){
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (!swapped && autoSwitch.get() && (!mc.player.isUsingItem() || !notOnUse.get())) {
            if (block.isReady()) {
                ...
                if (!slot.found() || mc.player.getInventory().selectedSlot == slot.slot()) return;
