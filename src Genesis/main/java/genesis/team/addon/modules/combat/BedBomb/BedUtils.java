...
import static genesis.team.addon.util.CombatUtil.PacketUtils.startPacketMine;
import static genesis.team.addon.util.CombatUtil.PredictionUtils.returnPredictBox;
import static genesis.team.addon.util.CombatUtil.PredictionUtils.returnPredictVec;
import static genesis.team.addon.util.InfoUtil.EntityInfo.getBlockPos;
...
public class BedUtils {
    ...
    public static boolean canBed(BlockPos canPlace, BlockPos replace) {
        return BlockInfo.getBlock(canPlace) instanceof BedBlock && BlockInfo.getBlock(replace) instanceof BedBlock || BlockUtils.canPlace(canPlace) && mc.world.getBlockState(replace).getMaterial().isReplaceable();
    }

    public static void packetMine(BlockPos blockpos, boolean autoSwap, Task task) {
        task.run(() -> {
            FindItemResult best = InvUtils.findFastestTool(BlockInfo.getState(blockpos));
    ...
    public static BlockPos getTrapBlock(PlayerEntity target, double distance) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            BlockPos pos = EntityInfo.getBlockPos(target).up().offset(direction);
    ...
    public static boolean shouldBurrowBreak() {
        ...
        return BlockInfo.isCombatBlock(b) && mc.player.getPos().distanceTo(BlockInfo.getCenterVec3d(b)) < 4.5;
    }

    public static boolean shouldTrapBreak() {
        return EntityInfo.isSurrounded(BedBomb.target) && EntityInfo.isTrapped(BedBomb.target) && getTrapBlock(BedBomb.target, 4.5) != null;
    }
    ...
    public static boolean shouldStringBreak() {
        ...
        for (CardinalDirection d : CardinalDirection.values()) {
            ...
            if (BlockInfo.getBlock(cPos).asItem().equals(Items.STRING) && mc.player.getPos().distanceTo(BlockInfo.getCenterVec3d(cPos)) < 4.5)
                strings.add(cPos);

            if (BlockInfo.getBlock(cPos.offset(d.toDirection())).asItem().equals(Items.STRING) && mc.player.getPos().distanceTo(BlockInfo.getCenterVec3d(cPos.offset(d.toDirection()))) < 4.5)
    ...
    public static double getDamage(PlayerEntity player, Vec3d cVec, boolean predictMovement, boolean collision, int i, boolean ignoreTerrain) {
        if (player != null && EntityInfo.isCreative(player)) return 0;
    ...
    private static BlockHitResult raycast(RaycastContext context, boolean ignoreTerrain) {
        return BlockView.raycast(context.getStart(), context.getEnd(), context, (raycastContext, blockpos) -> {
            BlockState blockState = BlockInfo.getState(blockpos);
