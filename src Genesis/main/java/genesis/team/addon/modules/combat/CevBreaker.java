...
public class CevBreaker extends Module {
    ...
    public CevBreaker() {
        super(Genesis.Combat, "cev-breaker", "");
    }
    ...
    public void onTick(TickEvent.Post event) {
        ...
        if (instant.get()) {
            ...
            if (startTask.isCalled()) {
                ...
                if (ticks == delay.get()) {
                    if (crystalEntity != null && BlockInfo.getState(breakPos).isAir()) {
    ...
    private BlockPos validPos(PlayerEntity player) {
        ...
        if (BlockInfo.getState(pos.up(3)).isAir() && (BlockInfo.getState(pos.up(2)).isOf(Blocks.OBSIDIAN) || BlockInfo.getState(pos.up(2)).isOf(Blocks.AIR))) return pos.up(2);
        List<BlockPos> posList = new ArrayList<>();

        for (CardinalDirection direction : CardinalDirection.values()) {
            if (BlockInfo.getState(pos.offset(direction.toDirection()).up(2)).isAir() && (BlockInfo.getState(pos.offset(direction.toDirection()).up(1)).isOf(Blocks.OBSIDIAN) || BlockInfo.getState(pos.offset(direction.toDirection()).up(1)).isOf(Blocks.AIR))) posList.add(pos.offset(direction.toDirection()).up(1));
        }
