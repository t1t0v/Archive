...
public class MinecartAura extends Module {
    ...
    public MinecartAura() {
        super(Genesis.Px, "minecart-aura-", "Rapes players with tnt minecarts. (made by Kightinum)");
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (didRailPlace && minecartDelayLeft >= minecartDelay.get() && !mining/* && firstTime */) {
            if (BlockUtil.getBlock(targetPos) == Blocks.RAIL) {
                ...
                mc.player.networkHandler.sendPacket(new PlayerInteractBlockC2SPacket(Hand.MAIN_HAND, new BlockHitResult(playerPos, Direction.UP, targetPos, false), 0));
                if (debug.get()) {
