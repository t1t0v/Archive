package genesis.team.addon.modules.combat.AutoCrystal;
...
import genesis.team.addon.util.CombatUtil.PredictionUtils;
...
public class AutoCrystal extends Module {
    ...
    private final Setting<RenderUtils.RenderMode> renderMode = sgRender.add(new EnumSetting.Builder<RenderUtils.RenderMode>().name("render-mode").description("How the render are rendered.").defaultValue(RenderUtils.RenderMode.UpperSide).build());
    public final Setting<Double> height = sgRender.add(new DoubleSetting.Builder().name("height").description("Maximum damage anchors can deal to yourself.").defaultValue(0.99).sliderRange(0, 1).visible(() -> RenderUtils.visibleHeight(renderMode.get())).build());
    ...
    private final Setting<SettingColor> sideColor = sgRender.add(new ColorSetting.Builder().name("side-color").description("The side color of the block overlay.").defaultValue(new SettingColor(255, 255, 255, 45)).visible(() -> RenderUtils.visibleSide(shapeMode.get())).build());
    private final Setting<SettingColor> lineColor = sgRender.add(new ColorSetting.Builder().name("line-color").description("The line color of the block overlay.").defaultValue(new SettingColor(255, 255, 255)).visible(() -> RenderUtils.visibleLine(shapeMode.get())).build());
    ...
    public AutoCrystal() {
        super(Genesis.Combat, "auto-crystal", "Automatically places and attacks crystals.");
    }
    ...
    private void doPlace() {
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get()), (int) Math.ceil(placeRange.get()), (bp, blockState) -> {
            ...
            double x = BlockInfo.X(bp);
            double y = BlockInfo.Y(bp) + 1;
            double z = BlockInfo.Z(bp);
            ((IBox) box).set(x, y, z, x + 1, y + (placement112.get() ? 1 : 2), z + 1);

            if (intersectsWithEntities(box)) return;

            if (predict.get()) {
                Box bx = PredictionUtils.returnPredictBox(bestTarget, predictCollision.get(), predictIncrease.get());
    ...
    private void findTargets() {
        ...
        for (PlayerEntity player : mc.world.getPlayers()) {
            if (EntityInfo.isCreative(player) || player == mc.player) continue;

            if (!EntityInfo.isDead(player) && EntityInfo.isAlive(player) && Friends.get().shouldAttack(player) && player.distanceTo(mc.player) <= targetRange.get()) {
                targets.add(player);
            }
        }

        // Fake players
        //for (PlayerEntity player : FakePlayerManager.() {
          //  if (!EntityInfo.isDead(player) && EntityInfo.isAlive(player) && Friends.get().shouldAttack(player) && player.distanceTo(mc.player) <= targetRange.get()) {
           //     targets.add(player);
          //  }
       // }
    ...
    private void onRender3d(Render3DEvent event) {
        ...
        if (renderTimer > 0 && renderMode.get() != RenderUtils.RenderMode.None) {
            if (fade.get()) {
        ...
        if (breakRenderTimer > 0 && renderBreak.get() && !mc.world.getBlockState(breakRenderPos).isAir()) {
            ...
            if (fade.get()) {
                renderBlocks.sort(Comparator.comparingInt((o) -> -o.ticks));
                renderBlocks.forEach((renderBlock) -> renderBlock.render(ri, sideColor.get(), lineColor.get()));
            } else RenderUtils.render(ri, breakRenderPos, sideColor.get(), lineColor.get(), height.get());
    ...
    public boolean shouldFacePlace() {
        ...
        for (PlayerEntity target : targets) {
            if (!EntityInfo.isSurrounded(target) || isTrapped(target)) return false;
    ...
    public boolean shouldSurroundBreak(BlockPos crystal) {
        ...
        for (PlayerEntity target : targets) {
            ...
            BlockPos targetPos = EntityInfo.getBlockPos(target);
            ...
            for (Direction direction : Direction.values()) {
                ...
                for (Direction direction2 : Direction.values()) {
                    ...
                    if (BlockInfo.isCombatBlock(offsetPos) && crystal.equals(offsetPos.offset(direction2))) {
    ...
    public EndCrystalEntity getSbCrystal(BlockPos blockPos) {
        for (Entity entity : EntityInfo.getEntities()) {
            if (entity instanceof EndCrystalEntity crystal && EntityInfo.getBlockPos(crystal).equals(blockPos)) return crystal;
        }
                ...
                for (Direction direction2 : Direction.values()) {
                    ...
                    for (Entity entity : mc.world.getEntities()) {
                        if (entity instanceof EndCrystalEntity crystal) {
                            BlockPos crystalPos = EntityInfo.getBlockPos(crystal);
    ...
    public boolean trueSurround(LivingEntity entity) {
        BlockPos entityBlockPos = EntityInfo.getBlockPos(entity);

        return BlockInfo.isCombatBlock(entity.getBlockPos().add(1, 0, 0))
                || BlockInfo.isCombatBlock(entityBlockPos.add(-1, 0, 0))
                || BlockInfo.isCombatBlock(entityBlockPos.add(0, 0, 1))
                || BlockInfo.isCombatBlock(entityBlockPos.add(0, 0, -1));
    }

    public boolean isTrapped(LivingEntity t) {
        BlockPos p = t.getBlockPos().up();
        return BlockInfo.isBlastResist(p.south()) && BlockInfo.isBlastResist(p.north()) && BlockInfo.isBlastResist(p.west()) && BlockInfo.isBlastResist(p.east());
    }

    @EventHandler
    private void onRender2D(Render2DEvent event) {
        if (renderMode.get() == RenderUtils.RenderMode.None || renderTimer <= 0 || !damageText.get()) return;

        if (renderMode.get() != RenderUtils.RenderMode.UpperSide || renderMode.get() != RenderUtils.RenderMode.LowerSide) {
