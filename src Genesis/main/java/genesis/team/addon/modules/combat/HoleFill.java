...
public class HoleFill extends Module {
    ...
    public HoleFill() {
        super(Genesis.Combat, "hole-fill", "Automatically fills holes around target.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        for (PlayerEntity target : targets) {
            ArrayList<BlockPos> sphere = new ArrayList<>(BedUtils.getTargetSphere(target, horizontalRadius.get(), 6));
            ...
            for (BlockPos blockPos : sphere) {
                ...
                for (Direction direction : Direction.values()) {
                    ...
                    if (BlockInfo.isBlastResist(blockPos.offset(direction))) blocks++;

                    else if (direction == Direction.DOWN) continue;
                    else if (validHole(blockPos.offset(direction)) && air == null) {
                        for (Direction dir : Direction.values()) {
                            if (dir == direction.getOpposite() || dir == Direction.UP) continue;
                            if (BlockInfo.isBlastResist(blockPos.offset(direction).offset(dir))) blocks++;
                            else continue;
                        }
    ...
    private void findTargets() {
        ...
        for (PlayerEntity player : mc.world.getPlayers()) {
            if (EntityInfo.isCreative(player) || player == mc.player || EntityInfo.isSurrounded(player) && ignoreSurrounded.get()) continue;
        ...
        // Fake players
        for (PlayerEntity player : mc.world.getPlayers()) {
            if (EntityInfo.isSurrounded(player) && ignoreSurrounded.get()) continue;
            if (!EntityInfo.isDead(player) && EntityInfo.isAlive(player) && Friends.get().shouldAttack(player) && player.distanceTo(mc.player) <= targetRange.get()) {
                targets.add(player);
            }
        }
