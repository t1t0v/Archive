...
public class Surround extends Module {
    ...
    public Surround() {
        super(Genesis.Combat, "surround-plus", "Surrounds you in blocks to prevent crystal damage");
    }
    ...
    public void onBreakPacket(PacketEvent.Receive event) {
        if (fagMode.get()) {
            ...
            if (event.packet instanceof BlockBreakingProgressS2CPacket) {
                ...
                obb = mc.world.getBlockState(bbp).getBlock().getBlastResistance() >= 600 && BlockInfo.getHardness(bbp) > 0;
    ...
    private void onPostTick(TickEvent.Post event) {
        ...
        if (crystalDelay <= 0) {
            ...
            for (BlockPos p : posPlaceBlocks) {
                if (antiCrystal.get()) {
                    boolean mine = BlockInfo.isReplaceable(p) || antiCrystalMode.get() != ACMode.Break;
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        BlockPos blockPos = (notSafeBlock(BlockInfo.getBlock(EntityInfo.getBlockPos(mc.player))) ? EntityInfo.getBlockPos(mc.player).up() : EntityInfo.getBlockPos(mc.player));
