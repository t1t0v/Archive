...
import genesis.team.addon.util.CombatUtil.TimerUtils;
...
public class PistonAura extends Module {
    ...
    public PistonAura() {
        super(Genesis.Combat, "piston-aura", "Automatically pushing crystals into enemy by piston.");
        get = this;
    }
    ...
    public void onTick(TickEvent.Post event) {
        ...
        switch (stage) {
            ...
            case BreakBlock -> {
                ...
                Interaction.doSwing(swing.get(), packetSwing.get(), Hand.MAIN_HAND);
    ...
    private void doPlace(FindItemResult itemResult, BlockPos blockPos) {
        ...
        mc.interactionManager.interactBlock(mc.player, hand, new BlockHitResult(Vec3dInfo.closestVec3d(blockPos), Direction.DOWN, blockPos, false));
    ...
    private boolean canCrystal(BlockPos blockPos) {
        if (Interaction.hasEntity(new Box(blockPos))) return false;
    ...
    private boolean canPlace(List<BlockPos> blockPoses) {
        for (BlockPos blockPos : blockPoses) {
            ...
            if (PlayerUtils.distanceTo(Vec3dInfo.closestVec3d(blockPos)) > placeRange.get()) return false;
        }
    ...
    private boolean canPlace(BlockPos blockPos, boolean ignoreEntity) {
        ...
        return !Interaction.hasEntity(new Box(blockPos), entity -> entity instanceof PlayerEntity || entity instanceof EndCrystalEntity || entity instanceof TntEntity);
    }
    ...
    public void onRender(Render3DEvent event) {
        ...
        RenderUtils.render(ri, crystalPos, sideColor.get(), lineColor.get(), height.get());
        RenderUtils.render(ri, pistonPos, sideColor.get(), lineColor.get(), height.get());
        RenderUtils.render(ri, blockPos, sideColor.get(), lineColor.get(), height.get());
        RenderUtils.render(ri, trap.get() && canPlace(trapPos, false) ? trapPos : null, sideColor.get(), lineColor.get(), height.get());
    }
    ...
    public class Positions {
        ...
        public static class Triplet {
            ...
            public double distance() {
                return PlayerUtils.distanceTo(Vec3dInfo.closestVec3d(blockPos.get(2)));
            }
        }
    }
}
