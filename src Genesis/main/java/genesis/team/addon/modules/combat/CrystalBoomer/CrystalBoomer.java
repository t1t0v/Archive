package genesis.team.addon.modules.combat.CrystalBoomer;

import genesis.team.addon.Genesis;
import genesis.team.addon.modules.render.TotemParticle;
...
public class CrystalBoomer extends Module {
    ...
    public CrystalBoomer() {
        super(Genesis.Px, "crystal-boomer", "v popu palchik");
    }
    ...
    public final Setting<TimingMode> timing = sgGeneral.add(new EnumSetting.Builder<TimingMode>().name("timing").description("timing").defaultValue(TimingMode.Adaptive).build());
    public final Setting<RotationMode> rotate = sgGeneral.add(new EnumSetting.Builder<RotationMode>().name("rotate").description("rotate").defaultValue(RotationMode.Off).build());
    public final Setting<Rotate> rotateMethod = sgGeneral.add(new EnumSetting.Builder<Rotate>().name("rotate-method").description("method?").defaultValue(Rotate.Packet).build());
    public final Setting<Boolean> inhibit = sgGeneral.add(new BoolSetting.Builder().name("inhibit").description("prevent unnecessary attacks").defaultValue(false).build());
    public final Setting<Boolean> limit = sgGeneral.add(new BoolSetting.Builder().name("limit").description("reduces attacks").defaultValue(false).build());
    public final Setting<YawStepMode> yawStep = sgGeneral.add(new EnumSetting.Builder<YawStepMode>().name("yawStep").description("ne usay naverno zalupa").defaultValue(YawStepMode.Off).build());
    ...
    public final Setting<Integer> yawTicks = sgGeneral.add(new IntSetting.Builder().name("yawTicks").description("rotate delay?").defaultValue(1).min(0).sliderMax(5).build());
    public final Setting<Boolean> strictDirection = sgGeneral.add(new BoolSetting.Builder().name("strictDirection").description("use this on strict servers").defaultValue(false).build());

    public final SettingGroup sgPlacements = settings.createGroup("Placements");
    public final Setting<Boolean> check = sgPlacements.add(new BoolSetting.Builder().name("check").description("proverka na nalicheye mam aitexa").defaultValue(true).build());
    public final Setting<DirectionMode> directionMode = sgPlacements.add(new EnumSetting.Builder<DirectionMode>().name("directionMode").description("stav Normal on zaebis").defaultValue(DirectionMode.Normal).build());
    public final Setting<Boolean> protocol = sgPlacements.add(new BoolSetting.Builder().name("protocol").description("1 13 crystal plase mod").defaultValue(false).build());
    public final Setting<Boolean> liquids = sgPlacements.add(new BoolSetting.Builder().name("liquids").description("ne ebu no vkluchayu").defaultValue(false).build());
    public final Setting<Boolean> fire = sgPlacements.add(new BoolSetting.Builder().name("fire").description("skashi net ognyu").defaultValue(false).build());
    public final Setting<Boolean> attackIntersects = sgPlacements.add(new BoolSetting.Builder().name("attackIntersects").description("huinya laguchaya").defaultValue(true).build());

    public final SettingGroup sgSupport = settings.createGroup("Support");
    public final Setting<Boolean> surroundBreak = sgSupport.add(new BoolSetting.Builder().name("surround-break").description("nah nada esli est citi break").defaultValue(true).build());
    public final Setting<Boolean> withMineProgress = sgSupport.add(new BoolSetting.Builder().name("with-mine-progress").description("nah nada esli est citi break").defaultValue(true).visible(surroundBreak::get).build());
    private final Setting<Double> breakingProgress = sgSupport.add(new DoubleSetting.Builder().name("breaking-progress").description("Tnah nada esli est citi break").defaultValue(0.9).min(0).sliderMax(1).visible(() -> withMineProgress.get() && surroundBreak.get()).build());
    public final Setting<Boolean> surroundHold = sgSupport.add(new BoolSetting.Builder().name("surround-hold").description("suicide mode").defaultValue(true).build());
    public final Setting<Boolean> support = sgSupport.add(new BoolSetting.Builder().name("support").description("podderzhivayet tebya v trudnyye momenty v zhizni").defaultValue(true).build());
    private final Setting<Keybind> forceSupport = sgSupport.add(new KeybindSetting.Builder().name("force-support").description("Starts support when this button is pressed.").defaultValue(Keybind.none()).build());
    public final Setting<Integer> supportDelay = sgSupport.add(new IntSetting.Builder().name("support-delay").description("The delay between placing blocks in ticks.").defaultValue(0).min(0).sliderMax(20).build());

    public final SettingGroup sgSpeeds = settings.createGroup("Speeds");
    public final Setting<ConfirmMode> confirm = sgSpeeds.add(new EnumSetting.Builder<ConfirmMode>().name("confirm").description("use this on 2b2t to improve consistency while chasing").defaultValue(ConfirmMode.Off).build());
    public final Setting<Integer> delay = sgSpeeds.add(new IntSetting.Builder().name("delay").description("The delay between placing crystal in ticks").defaultValue(0).min(0).sliderMax(20).build());
    ...
    public final Setting<Integer> attackFactor = sgSpeeds.add(new IntSetting.Builder().name("attackFactor").description("crystal pod snusom").defaultValue(20).min(0).sliderMax(20).build());
    public final Setting<Double> breakSpeed = sgSpeeds.add(new DoubleSetting.Builder().name("breakSpeed").description("breakSpeed").defaultValue(20).min(0.1).sliderMax(20).build());
    public final Setting<Double> placeSpeed = sgSpeeds.add(new DoubleSetting.Builder().name("placeSpeed").description("placeSpeed").defaultValue(20).min(0.1).sliderMax(20).build());
    public final Setting<SyncMode> syncMode = sgSpeeds.add(new EnumSetting.Builder<SyncMode>().name("syncMode").description("zabul cho za zalupa").defaultValue(SyncMode.Merge).build());
    public final Setting<Double> mergeOffset = sgSpeeds.add(new DoubleSetting.Builder().name("mergeOffset").description("huita use strict").defaultValue(0.7).sliderMax(1).build());
    ...
    public final Setting<Boolean> autoSwap = sgSwap.add(new BoolSetting.Builder().name("autoSwap").description("karoch samo kristall berot").defaultValue(true).build());
    ...
    public final Setting<Double> swapDelay = sgSwap.add(new DoubleSetting.Builder().name("swapDelay").description("delay between swap").defaultValue(0).min(0.0).sliderMax(10).build());
    public final Setting<Double> switchDelay = sgSwap.add(new DoubleSetting.Builder().name("switchDelay").description("delay between switch").defaultValue(0).min(0.0).sliderMax(10).build());
    public final Setting<Boolean> antiWeakness = sgSwap.add(new BoolSetting.Builder().name("antiWeakness").description("zalupa usay").defaultValue(false).build());
    ...
    public final Setting<Double> security = sgDamages.add(new DoubleSetting.Builder().name("security").description("maximum self damage for breaking enemy crystals").defaultValue(1).min(0.0).sliderMax(36).build());
    public final Setting<Double> compromise = sgDamages.add(new DoubleSetting.Builder().name("compromise").description("maximum self damage for breaking enemy crystals").defaultValue(1).min(0.0).sliderMax(36).build());
    public final Setting<Double> minPlaceDamage = sgDamages.add(new DoubleSetting.Builder().name("minPlaceDamage").description("minPlaceDamage").defaultValue(4.5).min(0.0).sliderMax(36).build());
    public final Setting<Double> maxSelfPlace = sgDamages.add(new DoubleSetting.Builder().name("maxSelfPlace").description("maxSelfPlace").defaultValue(12).min(0.0).sliderMax(36).build());
    public final Setting<Double> suicideHealth = sgDamages.add(new DoubleSetting.Builder().name("suicideHealth").description("suicideHealth").defaultValue(2).min(0.0).sliderMax(36).build());
    public final Setting<Double> faceplaceHealth = sgDamages.add(new DoubleSetting.Builder().name("facePlaceHealth").description("facePlaceHealth").defaultValue(4).min(0.0).sliderMax(36).build());
    ...
    public final Setting<Boolean> armorBreaker = sgDamages.add(new BoolSetting.Builder().name("armorBreaker").description("armorBreaker").defaultValue(false).build());
    public final Setting<Double> minStackDamage = sgDamages.add(new DoubleSetting.Builder().name("min-stack-damage").description("ne ebu").defaultValue(100).min(0.0).sliderMax(560).visible(armorBreaker::get).build());

    public final SettingGroup sgAntiFriendPop = settings.createGroup("AntiFriendPop");
    private final Setting<AntiFriendPop> antiFriendPop = sgAntiFriendPop.add(new EnumSetting.Builder<AntiFriendPop>().name("anti-friend-pop").description("huita laguchaya").defaultValue(AntiFriendPop.None).build());
    ...
    public final Setting<Boolean> collision = sgPrediction.add(new BoolSetting.Builder().name("collision").description("simulate collision when predicting motion").defaultValue(true).build());
    public final Setting<Boolean> predict = sgPrediction.add(new BoolSetting.Builder().name("predict").description("the amount of ticks to predict enemy motion by").defaultValue(true).build());
    public final Setting<Boolean> predictPops = sgPrediction.add(new BoolSetting.Builder().name("predictPops").description("ne ebu ne uzayte").defaultValue(true).build());
    public final Setting<Boolean> terrainIgnore = sgPrediction.add(new BoolSetting.Builder().name("terrainIgnore").description("ignore u mom").defaultValue(false).build());
    ...
    public final Setting<Boolean> noMineSwitch = sgPause.add(new BoolSetting.Builder().name("noMineSwitch").description("logichno").defaultValue(false).build());
    public final Setting<Boolean> noGapSwitch = sgPause.add(new BoolSetting.Builder().name("noGapSwitch").description("logichno").defaultValue(false).build());
    public final Setting<Boolean> rightClickGap = sgPause.add(new BoolSetting.Builder().name("rightClickGap").description("logichno").defaultValue(false).build());
    public final Setting<Boolean> disableWhenKA = sgPause.add(new BoolSetting.Builder().name("disableWhenKA").description("logichno").defaultValue(false).build());
    public final Setting<Double> disableUnderHealth = sgPause.add(new DoubleSetting.Builder().name("disableUnderHealth").description("esli hp malo crystalke pizda").defaultValue(2).min(0.0).sliderMax(10).build());
    public final Setting<Boolean> disableOnTP = sgPause.add(new BoolSetting.Builder().name("disableOnTP").description("logichno").defaultValue(false).build());
    ...
    private final AtomicBoolean shouldRunThread = new AtomicBoolean(true);
    ...
    private void handleSequential() {
        ...
        if ((mc.player.getHealth() + mc.player.getAbsorptionAmount() < disableUnderHealth.get()) || (disableWhenKA.get() && Modules.get().isActive(KillAura.class)) || (noGapSwitch.get() && mc.player.getStackInHand(mc.player.getActiveHand()).getItem().isFood()) || (noMineSwitch.get() && mc.interactionManager.isBreakingBlock() && mc.player.getMainHandStack().getItem() instanceof ToolItem)) {
            rotationVector = null;
            return;
        }
            ...
            if (mc.world.getEntityById(packetSpawnObject.getId()) instanceof EndCrystalEntity) {
                placeLocations.forEach((pos, time) -> {
                    if (getDistance(pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, packetSpawnObject.getX(), packetSpawnObject.getY() - 1, packetSpawnObject.getZ()) < 1) {
                        ...
                        if (PlayerUtils.shouldPause(stopPlacingOnMine.get(), stopPlacingOnEat.get(), stopPlacingOnDrink.get())) return;
                        if ((mc.player.getHealth() + mc.player.getAbsorptionAmount() < disableUnderHealth.get()) || (disableWhenKA.get() && Modules.get().isActive(KillAura.class)) || (noGapSwitch.get() && mc.player.getActiveItem().getItem().isFood()) || (noMineSwitch.get() && mc.interactionManager.isBreakingBlock() && mc.player.getMainHandStack().getItem() instanceof ToolItem)) {
                            rotationVector = null;
                            return;
                        }
    ...
    public boolean placeCrystal(BlockPos pos, Direction facing) {
        if (pos != null) {
            ...
            isPlacing = true;
            if (postResult == null) {
                mc.player.networkHandler.sendPacket(new PlayerInteractBlockC2SPacket(isOffhand() ? Hand.OFF_HAND : Hand.MAIN_HAND, new BlockHitResult(mc.player.getPos(), facing, pos, true), 0));
            } else {
                mc.player.networkHandler.sendPacket(new PlayerInteractBlockC2SPacket(isOffhand() ? Hand.OFF_HAND : Hand.MAIN_HAND, postResult, 0));
                if (renderSwing.get()) mc.player.swingHand(isOffhand() ? Hand.OFF_HAND : Hand.MAIN_HAND);
            }
