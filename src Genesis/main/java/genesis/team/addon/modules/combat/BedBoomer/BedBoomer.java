package genesis.team.addon.modules.combat.BedBoomer;
...
public class BedBoomer extends Module {
    public BedBoomer(){
        super(Genesis.Px, "bed-boomer", "");
    }
    ...
    private final Setting<Boolean> packetPlace = sgPlace.add(new BoolSetting.Builder().name("packet-place").description("Will not place and break beds if they will kill you.").defaultValue(true).build());
    ...
    private final Pool<MineBlock> renderMineBlockPool = new Pool<>(MineBlock::new);
    private final List<MineBlock> renderMineBlocks = new ArrayList<>();
    ...
    public void onActivate() {
        ...
        if (!renderMineBlocks.isEmpty()){
            for (MineBlock renderBlock : renderMineBlocks) renderMineBlockPool.free(renderBlock);
            renderMineBlocks.clear();
        }
    ...
    private void onTickPre(TickEvent.Pre event){
        ...
        renderMineBlocks.forEach(MineBlock::tick);
        renderMineBlocks.removeIf(renderBlock -> renderBlock.ticks <= 0);

        if (mc.world.getDimension().bedWorks() && checkDimension.get()){
        ...
        if ((breakBurrow.get().isPressed() || autoMineBurrow.get()) && !(BlockUtil.getBlock(playerArray.get(0).getBlockPos()) instanceof BedBlock) && EntityUtil.isBurrowed(playerArray.get(0)) && BlockUtil.getState(playerArray.get(0).getBlockPos()).getHardness(mc.world, playerArray.get(0).getBlockPos()) != -1 && BlockUtil.distance(new BlockPos(mc.player.getEyePos()),  playerArray.get(0).getBlockPos()) <= placeRange.get()){
            ...
            if (BlockUtil.getState(playerArray.get(0).getBlockPos()).getHardness(mc.world, playerArray.get(0).getBlockPos()) == 0){
                ...
                renderMineBlocks.add(renderMineBlockPool.get().set(playerArray.get(0).getBlockPos(), mineFadeTick.get()));
        ...
        try {
            if ((breakTrap.get().isPressed() || autoMineTrap.get()) && isSelfTraped(playerArray.get(0)) && !(BlockUtil.getBlock(getBestTrapPos(playerArray.get(0))) instanceof BedBlock) && BlockUtil.distance(new BlockPos(mc.player.getEyePos()), getBestTrapPos(playerArray.get(0))) <= placeRange.get()){
                ...
                if (getBestTrapPos(playerArray.get(0)) != null){
                    ...
                    renderMineBlocks.add(renderMineBlockPool.get().set(getBestTrapPos(playerArray.get(0)), mineFadeTick.get()));
        ...
        if ((breakTop.get().isPressed() || autoMineTop.get()) && !(BlockUtil.getBlock(playerArray.get(0).getBlockPos().up(2)) instanceof BedBlock) && !BlockUtil.isAir(playerArray.get(0).getBlockPos().up(2)) && BlockUtil.getState(playerArray.get(0).getBlockPos().up(2)).getHardness(mc.world, playerArray.get(0).getBlockPos().up(2)) != -1 && BlockUtil.distance(new BlockPos(mc.player.getEyePos()),  playerArray.get(0).getBlockPos().up(2)) <= placeRange.get()){
            ...
            renderMineBlocks.add(renderMineBlockPool.get().set(playerArray.get(0).getBlockPos().up(2), mineFadeTick.get()));
            ...
            if (!EntityUtil.isInHole(true, finalTarget2) || EntityUtil.getPlayerSpeed(finalTarget2) > 1 || BlockUtil.distance(new BlockPos(mc.player.getEyePos()), finalTarget2.getBlockPos()) >= placeRange.get()){
                Runnable task = () -> {
                    try {
                        ...
                        array = sortBedPos(getPosAround(), finalTarget2);
                ...
                if (thread.getThreadGroup().activeCount() > 1 || thread.getState() == Thread.State.TERMINATED){
                    thread.start();
                    if (debugChat.get()) info("Time: " + (System.currentTimeMillis() - time) + "; BestDamage: " + currentDamage + "; Direction: " + (direction == null ? "None" : direction.toString()));
                }else {
                    thread.interrupt();
                }
    ...
    private boolean placeBed(Pair<BlockPos, BlockPos> pair, PlayerEntity target){
        ...
        Rotations.rotate(direction.asRotation(), Rotations.getPitch(pair.getLeft()), () -> {
            ...
            if (testPlace.get()){
                PlayerUtil.placeBlock(packetPlace.get(), bed.slot(), false, 0, 0, false, false, Hand.MAIN_HAND, pair.getRight(), !oneDotTwelve.get(), !oneDotTwelve.get(), true);
            }
            else if (meteorPlace.get()){
                PlayerUtil.placeBlockNoRotate(packetPlace.get(), bed.slot(), Hand.MAIN_HAND, pair.getRight());
            }
            else {
                mc.interactionManager.interactBlock(mc.player, Hand.MAIN_HAND, new BlockHitResult(mc.player.getPos(), Direction.DOWN, pair.getRight(), true));
            }
    ...
    private void breakBed(BlockPos pos) {
        ...
        if (testBreak.get()){
            ...
            mc.interactionManager.interactBlock(mc.player, handMode.get(), new BlockHitResult(mc.player.getPos(), Direction.DOWN, pos, false));
        }
    ...
    private BlockPos findBreak(PlayerEntity target){
        List<BlockPos> array = BlockUtil.getSphere(new BlockPos(new BlockPos(mc.player.getEyePos())), xBreakRange.get().floatValue(), yBreakRange.get().intValue(), false, true, 0).stream().filter(blockPos -> BlockUtil.getBlock(blockPos) instanceof BedBlock).collect(Collectors.toList());
        ...
        array = array.stream().filter(blockPos -> {
            ...
            return !lethalDamage.get() || !(PlayerUtils.getTotalHealth() - bedDamage(mc.player, blockPos, ignoreBed.get()) < minHealth.get());
    ...
    private double bedDamage(PlayerEntity player, BlockPos pos, boolean ignoreBed) {
        if (player == null || player.getAbilities().creativeMode && !(player instanceof PlayerEntity)) return 0;
    ...
    private boolean canBedPlace(BlockPos blockPos){
        Box box = new Box(blockPos.getX(), blockPos.getY(), blockPos.getZ(), blockPos.getX() + 1, blockPos.getY() + 0.6, blockPos.getZ() + 1);
        if (!BlockUtil.isAir(blockPos) && BlockUtil.getBlock(blockPos) != Blocks.LAVA && BlockUtil.getBlock(blockPos) != Blocks.FIRE) return false;
        if ((!BlockUtil.isAir(blockPos.west()) && BlockUtil.getBlock(blockPos.west()) != Blocks.LAVA && BlockUtil.getBlock(blockPos.west()) != Blocks.FIRE) &&
            (!BlockUtil.isAir(blockPos.east()) && BlockUtil.getBlock(blockPos.east()) != Blocks.LAVA && BlockUtil.getBlock(blockPos.east()) != Blocks.FIRE) &&
            (!BlockUtil.isAir(blockPos.south()) && BlockUtil.getBlock(blockPos.south()) != Blocks.LAVA && BlockUtil.getBlock(blockPos.south()) != Blocks.FIRE) &&
            (!BlockUtil.isAir(blockPos.north()) && BlockUtil.getBlock(blockPos.north()) != Blocks.LAVA && BlockUtil.getBlock(blockPos.north()) != Blocks.FIRE)
        ) return false;
    ...
    private Color sidetop;
    private Color sidebottom;
    private Color linetop;
    private Color linebottom;

    private Color pillowSide;
    private Color pillowLine;
    private Color spreadSide;
    private Color spreadLine;
    private Color woodSide;
    private Color woodLine;
        ...
        if (render.get()){
            if (!renderDamageColor.get() && !rainbowColor.get()){
                switch (renderMode.get()){
                    case BedForm -> {
                        sidetop = sideColorTop.get();
                        sidebottom = sideColorBottom.get();
                        linetop = lineColorTop.get();
                        linebottom = lineColorBottom.get();
                    }
                    case Lines -> {
                        pillowSide = pillowSideColor.get();
                        pillowLine = pillowLineColor.get();
                        spreadSide = bedSpreadSideColor.get();
                        spreadLine = bedSpreadLineColor.get();
                        woodSide = woodSideColor.get();
                        woodLine = woodLineColor.get();
                    }
                    case SingleBox -> {
                        side = sideColor.get();
                        line = lineColor.get();
                ...
                switch (renderMode.get()){
                    case BedForm -> {
                        sidetop = new Color(TotemParticle.rainbowColor().getRed(), TotemParticle.rainbowColor().getGreen(), TotemParticle.rainbowColor().getBlue(), sideAlpha.get());
                        sidebottom = new Color(TotemParticle.rainbowColor().getRed(), TotemParticle.rainbowColor().getGreen(), TotemParticle.rainbowColor().getBlue(), sideAlpha.get());
                        linetop = new Color(TotemParticle.rainbowColor().getRed(), TotemParticle.rainbowColor().getGreen(), TotemParticle.rainbowColor().getBlue(), lineAlpha.get());
                        linebottom = new Color(TotemParticle.rainbowColor().getRed(), TotemParticle.rainbowColor().getGreen(), TotemParticle.rainbowColor().getBlue(), lineAlpha.get());
                    }
                    case Lines -> {
                        pillowSide = new Color(TotemParticle.rainbowColor().getRed(), TotemParticle.rainbowColor().getGreen(), TotemParticle.rainbowColor().getBlue(), sideAlpha.get());
                        pillowLine = new Color(TotemParticle.rainbowColor().getRed(), TotemParticle.rainbowColor().getGreen(), TotemParticle.rainbowColor().getBlue(), lineAlpha.get());
                        spreadSide = new Color(TotemParticle.rainbowColor().getRed(), TotemParticle.rainbowColor().getGreen(), TotemParticle.rainbowColor().getBlue(), sideAlpha.get());
                        spreadLine = new Color(TotemParticle.rainbowColor().getRed(), TotemParticle.rainbowColor().getGreen(), TotemParticle.rainbowColor().getBlue(), lineAlpha.get());
                        woodSide = new Color(TotemParticle.rainbowColor().getRed(), TotemParticle.rainbowColor().getGreen(), TotemParticle.rainbowColor().getBlue(), sideAlpha.get());
                        woodLine = new Color(TotemParticle.rainbowColor().getRed(), TotemParticle.rainbowColor().getGreen(), TotemParticle.rainbowColor().getBlue(), lineAlpha.get());
                    }
                    case SingleBox -> {
                        side = new Color(TotemParticle.rainbowColor().getRed(), TotemParticle.rainbowColor().getGreen(), TotemParticle.rainbowColor().getBlue(), sideAlpha.get());
                        line = new Color(TotemParticle.rainbowColor().getRed(), TotemParticle.rainbowColor().getGreen(), TotemParticle.rainbowColor().getBlue(), lineAlpha.get());
                    }
                }
            }
            else {
                if (currentDamage >= 14){
                    switch (renderMode.get()){
                        case BedForm -> {
                            sidetop = maxDamColor.get().copy().a(sideAlpha.get());
                            sidebottom = maxDamColor.get().copy().a(sideAlpha.get());
                            linetop = maxDamColor.get().copy().a(lineAlpha.get());
                            linebottom = maxDamColor.get().copy().a(lineAlpha.get());
                        }
                        case Lines -> {
                            pillowSide = maxDamColor.get().copy().a(sideAlpha.get());
                            pillowLine = maxDamColor.get().copy().a(lineAlpha.get());
                            spreadSide = maxDamColor.get().copy().a(sideAlpha.get());
                            spreadLine = maxDamColor.get().copy().a(lineAlpha.get());
                            woodSide = maxDamColor.get().copy().a(sideAlpha.get());
                            woodLine = maxDamColor.get().copy().a(lineAlpha.get());
                        }
                        case SingleBox -> {
                            side = maxDamColor.get().copy().a(sideAlpha.get());
                            line = maxDamColor.get().copy().a(lineAlpha.get());
                        }
                    }
                }
                if (currentDamage < 14 && currentDamage >= 12){
                    switch (renderMode.get()){
                        case BedForm -> {
                            sidetop = highDamColor.get().copy().a(sideAlpha.get());
                            sidebottom = highDamColor.get().copy().a(sideAlpha.get());
                            linetop = highDamColor.get().copy().a(lineAlpha.get());
                            linebottom = highDamColor.get().copy().a(lineAlpha.get());
                        }
                        case Lines -> {
                            pillowSide = highDamColor.get().copy().a(sideAlpha.get());
                            pillowLine = highDamColor.get().copy().a(lineAlpha.get());
                            spreadSide = highDamColor.get().copy().a(sideAlpha.get());
                            spreadLine = highDamColor.get().copy().a(lineAlpha.get());
                            woodSide = highDamColor.get().copy().a(sideAlpha.get());
                            woodLine = highDamColor.get().copy().a(lineAlpha.get());
                        }
                        case SingleBox -> {
                            side = highDamColor.get().copy().a(sideAlpha.get());
                            line = highDamColor.get().copy().a(lineAlpha.get());
                        }
                    }
                }
                if (currentDamage < 12 && currentDamage >= 10){
                    switch (renderMode.get()){
                        case BedForm -> {
                            sidetop = mediumDamColor.get().copy().a(sideAlpha.get());
                            sidebottom = mediumDamColor.get().copy().a(sideAlpha.get());
                            linetop = mediumDamColor.get().copy().a(lineAlpha.get());
                            linebottom = mediumDamColor.get().copy().a(lineAlpha.get());
                        }
                        case Lines -> {
                            pillowSide = mediumDamColor.get().copy().a(sideAlpha.get());
                            pillowLine = mediumDamColor.get().copy().a(lineAlpha.get());
                            spreadSide = mediumDamColor.get().copy().a(sideAlpha.get());
                            spreadLine = mediumDamColor.get().copy().a(lineAlpha.get());
                            woodSide = mediumDamColor.get().copy().a(sideAlpha.get());
                            woodLine = mediumDamColor.get().copy().a(lineAlpha.get());
                        }
                        case SingleBox -> {
                            side = mediumDamColor.get().copy().a(sideAlpha.get());
                            line = mediumDamColor.get().copy().a(lineAlpha.get());
                        }
                    }
                }
                if (currentDamage < 10 && currentDamage >= 7){
                    switch (renderMode.get()){
                        case BedForm -> {
                            sidetop = belowMediumDamColor.get().copy().a(sideAlpha.get());
                            sidebottom = belowMediumDamColor.get().copy().a(sideAlpha.get());
                            linetop = belowMediumDamColor.get().copy().a(lineAlpha.get());
                            linebottom = belowMediumDamColor.get().copy().a(lineAlpha.get());
                        }
                        case Lines -> {
                            pillowSide = belowMediumDamColor.get().copy().a(sideAlpha.get());
                            pillowLine = belowMediumDamColor.get().copy().a(lineAlpha.get());
                            spreadSide = belowMediumDamColor.get().copy().a(sideAlpha.get());
                            spreadLine = belowMediumDamColor.get().copy().a(lineAlpha.get());
                            woodSide = belowMediumDamColor.get().copy().a(sideAlpha.get());
                            woodLine = belowMediumDamColor.get().copy().a(lineAlpha.get());
                        }
                        case SingleBox -> {
                            side = belowMediumDamColor.get().copy().a(sideAlpha.get());
                            line = belowMediumDamColor.get().copy().a(lineAlpha.get());
                        }
                    }
                }
                if (currentDamage < 7 && currentDamage >= 4){
                    switch (renderMode.get()){
                        case BedForm -> {
                            sidetop = littleDamColor.get().copy().a(sideAlpha.get());
                            sidebottom = littleDamColor.get().copy().a(sideAlpha.get());
                            linetop = littleDamColor.get().copy().a(lineAlpha.get());
                            linebottom = littleDamColor.get().copy().a(lineAlpha.get());
                        }
                        case Lines -> {
                            pillowSide = littleDamColor.get().copy().a(sideAlpha.get());
                            pillowLine = littleDamColor.get().copy().a(lineAlpha.get());
                            spreadSide = littleDamColor.get().copy().a(sideAlpha.get());
                            spreadLine = littleDamColor.get().copy().a(lineAlpha.get());
                            woodSide = littleDamColor.get().copy().a(sideAlpha.get());
                            woodLine = littleDamColor.get().copy().a(lineAlpha.get());
                        }
                        case SingleBox -> {
                            side = littleDamColor.get().copy().a(sideAlpha.get());
                            line = littleDamColor.get().copy().a(lineAlpha.get());
                        }
                    }
                }
                if (currentDamage < 4){
                    switch (renderMode.get()){
                        case BedForm -> {
                            sidetop = lowDamColor.get().copy().a(sideAlpha.get());
                            sidebottom = lowDamColor.get().copy().a(sideAlpha.get());
                            linetop = lowDamColor.get().copy().a(lineAlpha.get());
                            linebottom = lowDamColor.get().copy().a(lineAlpha.get());
                        }
                        case Lines -> {
                            pillowSide = lowDamColor.get().copy().a(sideAlpha.get());
                            pillowLine = lowDamColor.get().copy().a(lineAlpha.get());
                            spreadSide = lowDamColor.get().copy().a(sideAlpha.get());
                            spreadLine = lowDamColor.get().copy().a(lineAlpha.get());
                            woodSide = lowDamColor.get().copy().a(sideAlpha.get());
                            woodLine = lowDamColor.get().copy().a(lineAlpha.get());
                        }
                        case SingleBox -> {
                            side = lowDamColor.get().copy().a(sideAlpha.get());
                            line = lowDamColor.get().copy().a(lineAlpha.get());
                        }
                    }
                }
            }
    ...
    private void onRender3D(Render3DEvent event) {
        if (render.get()){
            if (renderMine.get()) {
                if (!mineFade.get() && minePos != null && !BlockUtil.isAir(minePos) && !(BlockUtil.getBlock(minePos) instanceof BedBlock)) {
                    event.renderer.box(minePos, mineSideColor.get(), mineLineColor.get(), mineShapeMode.get(), 0);
                }
                else if (mineFade.get()){
                    renderMineBlocks.sort(Comparator.comparingInt(o -> -o.ticks));
                    renderMineBlocks.forEach(renderBlock -> renderBlock.render(event, mineSideColor.get(), mineSideColor2.get(), mineLineColor.get(), mineLineColor2.get(), mineShapeMode.get(), width.get()));
                }
            }
            switch (renderMode.get()){
                case BedForm -> {
                    if (!renderBlocks.isEmpty()) {
                        if (sidetop == null || sidebottom == null || linetop == null || linebottom == null) return;
                ...
                case DoubleBox -> {
                    ...
                    if (fade.get()) {
                        ...
                        renderBlocks.forEach(renderBlock -> renderBlock.render(event, side, line, shapeMode.get()));
                    } else {
                        ...
                        event.renderer.box(box2, side, line, shapeMode.get(), 0);
                    }
                }
                case SingleBox -> {
                    if (side == null || line == null) return;
                    if (fade.get()) {
                        renderBlocks.sort(Comparator.comparingInt(o -> o.ticks));
                        renderBlocks.forEach(renderBlock -> renderBlock.renderPart(event, boxMode.get(), side, line, shapeMode.get()));
                    } else {
                        if (headPos == null) return;
                        switch (boxMode.get()){
                            case Head -> {
                ...
                case Lines -> {
                    if (pillowSide == null || pillowLine == null || spreadSide == null || spreadLine == null || woodSide == null || woodLine == null) return;
                    renderBlocks.sort(Comparator.comparingInt(o -> o.ticks));
                    renderBlocks.forEach(renderBlock -> renderBlock.renderLines(event, pillowSide, spreadSide, woodSide, pillowLine, spreadLine, woodLine, shapeMode.get()));
                }
            }
            if (renderTimer.hasPassed(renderTick.get() * 50f)){
                headPos = null;
                feetPos = null;
                renderTimer.reset();
            }
    ...
    public static class MineBlock {
        ...
        public MineBlock set(BlockPos blockPos, int ticks) {
            pos.set(blockPos);
            this.ticks = ticks;
        ...
        public void render(Render3DEvent event, Color side1, Color side2, Color line1, Color line2, ShapeMode shapeMode, int width) {
            ...
            side1.a *= (double) ticks / 5;
            side2.a *= (double) ticks / 5;
            line1.a *= (double) ticks / 5;
            line2.a *= (double) ticks / 5;
    ...
    public class RenderBlock {
        ...
        public RenderBlock setBlock(BlockPos head, BlockPos feet, int tick) {
            ...
            sidesTop = sidetop;
            sidesBottom = sidebottom;
            linesTop = linetop;
            linesBottom = linebottom;
