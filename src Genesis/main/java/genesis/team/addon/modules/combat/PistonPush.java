...
public class PistonPush extends Module {
    ...
    public PistonPush() {
        super(Genesis.Combat, "piston-push", "pushes out of the hole.");
    }
    ...
    public void onTick(TickEvent.Post event) {
        ...
        switch (stage) {
            case Preparing -> {
                ...
                targetPos = EntityInfo.getBlockPos(target).up();
                ...
                if (antiSelf.get() && Interaction.hasEntity(new Box(targetPos), entity -> entity == mc.player)) stage = Stage.Toggle;
    ...
    private void doPlace(FindItemResult result, BlockPos blockPos) {
        if (BlockInfo.isNull(blockPos)) return;
    ...
    private BlockPos getRedstonePos(BlockPos blockPos) {
        ...
        if (BlockInfo.isNull(blockPos)) return null;

        for (Direction dir : Direction.values()) {
            if (Interaction.hasEntity(new Box(blockPos.offset(dir)))) continue;
    ...
    private BlockPos getPistonPos(BlockPos blockPos) {
        ...
        for (Direction dir : Direction.values()) {
            ...
            boolean canPush = BlockInfo.isAir(blockPos.up()) && BlockInfo.isAir(blockPos.offset(revert(dir))) && BlockInfo.isAir(blockPos.offset(revert(dir)).up());
    ...
    private boolean hasNull(BlockPos... blockPoses) {
        for (BlockPos blockPos : blockPoses) {
            if (BlockInfo.isNull(blockPos)) return true;
        }
    ...
    private boolean hasFar(BlockPos... blockPoses) {
        for (BlockPos blockPos : blockPoses) {
            if (PlayerUtils.distanceTo(BlockInfo.closestVec3d(blockPos)) > placeRange.get()) return true;
        }
    ...
    public void onRender(Render3DEvent event) {
        ...
        RenderUtils.render(ri, activatorPos, sideColor.get(), lineColor.get(), height.get());

        if (!holeFill.get()) return;
        RenderUtils.render(ri, obsidianPos, sideColor.get(), lineColor.get(), height.get());
    }
