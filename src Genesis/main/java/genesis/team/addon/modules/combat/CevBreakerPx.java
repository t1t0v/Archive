...
public class CevBreakerPx extends Module {
    public CevBreakerPx(){
        super(Genesis.Px, "cev-miner", "Place a crystal over the player's head and explodes");
    ...
    private void placeCrystal(BlockPos pos){
        if (mc.player.getOffHandStack().getItem() == Items.END_CRYSTAL){
            mc.interactionManager.interactBlock(mc.player, Hand.OFF_HAND, new BlockHitResult(mc.player.getPos(), Direction.UP, pos,true));
        }
    ...
    private final List<BlockPos> cevArray = new ArrayList<>(){{
    ...
    private final List<BlockPos> posArray = new ArrayList<>(){{
    ...
    private void onRender2D(Render2DEvent event) {
        ...
        if (target != null && minePos != null && !mc.world.getBlockState(minePos).isAir() && mineMode.get().equals(CityDestroyer.MineMode.Vanilla)) {
