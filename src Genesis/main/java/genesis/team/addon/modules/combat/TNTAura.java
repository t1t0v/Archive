...
public class TNTAura extends Module {
    ...
    public TNTAura() {
        super(Genesis.Combat, "TNT-Aura", "Placing & igniting TNT around enemy");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (holePause.get() && !EntityInfo.isSurrounded(target) && !toggled) {
            obsidianPos.clear();
    ...
    public boolean tntBlockstate(BlockPos Pos) {
        return BlockInfo.getBlock(Pos) == Blocks.AIR
            || BlockInfo.getBlock(Pos) == Blocks.TNT;
    }

    public boolean allowTNT(LivingEntity target) {
        assert mc.world != null;

        return !BlockInfo.isAir(target.getBlockPos().add(1, 2, 0)) && !BlockInfo.isAir(target.getBlockPos().add(-1, 2, 0)) && !BlockInfo.isAir(target.getBlockPos().add(0, 2, 1)) && !BlockInfo.isAir(target.getBlockPos().add(0, 2, -1)) && !BlockInfo.isAir(target.getBlockPos().add(0, 3, 0));
    }

    public boolean mineBlockstate(BlockPos Pos) {
        return BlockInfo.getBlock(Pos) == Blocks.AIR
            || BlockInfo.getBlock(Pos) == Blocks.TNT
            || BlockInfo.getBlock(Pos) == Blocks.BEDROCK;
    ...
    private boolean isBurrowed(LivingEntity target) {
        ...
        return !BlockInfo.isAir(EntityInfo.getBlockPos(target));
    }
