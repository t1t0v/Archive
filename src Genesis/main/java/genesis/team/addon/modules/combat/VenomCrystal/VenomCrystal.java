package genesis.team.addon.modules.combat.VenomCrystal;
...
public class VenomCrystal extends Module {
    ...
    // Target
    public final Setting<Object2BooleanMap<EntityType<?>>> entities = sgTarget.add(new EntityTypeListSetting.Builder().name("entities").description("The entities to attack.").defaultValue(Utils.asO2BMap(EntityType.PLAYER)).onlyAttackable().build());
    ...
    public VenomCrystal() {
        super(Genesis.Px, "support-crystal", "Auto crystal made by tesako.");
    }
    ...
    private void onTick(SendMovementPacketsEvent.Pre event) {
        ...
        weak = false;
        if(!effects.isEmpty()) {
            boolean strong = effects.containsKey(StatusEffects.STRENGTH) && effects.get(StatusEffects.STRENGTH).getAmplifier() == 1;
            //You can destroy crystals with your bare hands if you have strength 2 with weakness 1
    ...
    private void doPlace() {//TODO rework the order in which damages are calculated
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get()), (int) Math.ceil(verticalRange.get()), (blockPos, blockState) -> {
            ...
            for(LivingEntity target : targets) {
                ...
                boolean facePlace = facePlaceLimit;
        ...
        BlockIterator.after(() -> {
            if(bestPos.get() != null || bestSupportPos.get() != null) {
                ...
                if(rotationMode.get() == RotationMode.Place || rotationMode.get() == RotationMode.Both) {
                    ...
                    Rotations.rotate(rotation[0], rotation[1], 25, () -> mc.player.networkHandler.sendPacket(new PlayerInteractBlockC2SPacket(getHand(), result, 0)));

                } else mc.player.networkHandler.sendPacket(new PlayerInteractBlockC2SPacket(getHand(), result, 0));
