...
import com.example.addon.util.advanced.CityUtils;
import com.example.addon.util.other.Task;
import com.example.addon.util.BEntityUtils;
import com.example.addon.util.BPlayerUtils;
...
import static com.example.addon.util.basic.BlockInfo.isAir;
...
public class CityBreaker extends Module {
    ...
    private final SettingGroup sgNone = settings.createGroup("Notifications");
    ...
    private final Setting<Boolean> Instant = sgGeneral.add(new BoolSetting.Builder().name("Instant").description("Instant").defaultValue(true).build());
    ...
    public CityBreaker() {
        super(Addon.Alliance, "city-breaker", "Automatically breaks target's surround.");
    }
