...
import static com.example.addon.util.basic.EntityInfo.getBlockPos;
...
public class Notifications extends Module {
    ...
    private final Setting<Mode> notifications = sgNone.add(new EnumSetting.Builder<Mode>().name("notifications").defaultValue(Mode.Chat).build());
    ...
    public Notifications() {
        super(Addon.Alliance, "Notifications", "Sends messages in hud about different events.");
    }
        ...
        synchronized (totemPopMap) {
            for (PlayerEntity player : mc.world.getPlayers()) {
                ...
                if (player.deathTime > 0 || player.getHealth() <= 0) {
                    ...
                    if (deaths.get()) send(player.getEntityName() + " died after  " + pops +" pops!", notifications);
    ...
    public static void send(String msg, Setting<Mode> notify) {
        switch (notify.get()) {
