...
import com.example.addon.util.advanced.CrystalUtils;
import com.example.addon.util.advanced.Interaction;
import com.example.addon.util.advanced.RenderUtils;
import com.example.addon.util.basic.RenderInfo;
...
import static com.example.addon.util.advanced.PredictionUtils.returnPredictBox;
import static com.example.addon.util.advanced.RenderUtils.*;
import static com.example.addon.util.basic.BlockInfo.X;
import static com.example.addon.util.basic.BlockInfo.Y;
import static com.example.addon.util.basic.BlockInfo.Z;
import static com.example.addon.util.basic.BlockInfo.*;
import static com.example.addon.util.basic.EntityInfo.*;
...
public class AutoCrystal extends Module {
    ...
    //Predict
    public final Setting<Boolean> predict = sgPredict.add(new BoolSetting.Builder().name("predict-position").description("Predicts target position.").defaultValue(false).build());
    ...
    public AutoCrystal() {
        super(Addon.Alliance, "auto-crystal", "Automatically places and attacks crystals.");
    }
    ...
    private void findTargets() {
        ...
        // Fake players
        for (PlayerEntity player : FakePlayerManager.stream().toList()) {
            if (!isDead(player) && isAlive(player) && Friends.get().shouldAttack(player) && player.distanceTo(mc.player) <= targetRange.get()) {
                targets.add(player);
            }
        }
