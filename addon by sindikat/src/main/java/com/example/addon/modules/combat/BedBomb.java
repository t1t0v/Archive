...
import com.example.addon.modules.combat.AutoCrystal;
import com.example.addon.modules.info.Notifications;
import com.example.addon.util.basic.BlockInfo;
import com.example.addon.util.basic.EntityInfo;
import com.example.addon.util.basic.Vec3dInfo;
import com.example.addon.util.other.Task;
import com.example.addon.util.advanced.BedUtils;
import com.example.addon.util.advanced.RenderUtils;
...
public class BedBomb extends Module {
    ...
    private final Pool<BedUtils.RenderText> renderTextPool = new Pool<>(BedUtils.RenderText::new);
    private final List<BedUtils.RenderText> renderTexts = new ArrayList<>();

    private final Pool<BedUtils.RenderBlock> renderBlockPool = new Pool<>(BedUtils.RenderBlock::new);
    private final List<BedUtils.RenderBlock> renderBlocks = new ArrayList<>();

    private final Pool<BedUtils.RenderBreak> renderBreakPool = new Pool<>(BedUtils.RenderBreak::new);
    private final List<BedUtils.RenderBreak> renderBreaks = new ArrayList<>();

    public BedBomb() {
        super(Addon.Alliance, "bed-bomb", "Massive ownage incoming :massivetroll:");
    }
    ...
    public void onActivate() {
        ...
        for (BedUtils.RenderBlock renderBlock : renderBlocks) renderBlockPool.free(renderBlock);
        renderBlocks.clear();
        for (BedUtils.RenderBreak renderBreak : renderBreaks) renderBreakPool.free(renderBreak);
        renderBreaks.clear();
        for (BedUtils.RenderText renderText : renderTexts) renderTextPool.free(renderText);
        renderTexts.clear();
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        renderBlocks.forEach(BedUtils.RenderBlock::tick);
        renderBlocks.removeIf(renderBlock -> renderBlock.ticks <= 0);

        renderBreaks.forEach(BedUtils.RenderBreak::tick);
        renderBreaks.removeIf(renderBreak -> renderBreak.ticks <= 0);

        renderTexts.forEach(BedUtils.RenderText::tick);
        ...
        if ((lay.get() || forceLay.get().isPressed()) && EntityInfo.isSurrounded(target) && !EntityInfo.isFaceTrapped(target)) {
            calculateHolePos();
            ...
            return;
        } else if (!EntityInfo.isFaceTrapped(target)){
            smartLay = true;
            failTimes = -1;
        }
        ...
        if (EntityUtils.getTotalHealth(target) <= 11 && zeroTick.get() && !EntityInfo.isSurrounded(target)) {
            int i = placeDelay.get() <= 9 ? 0 : placeDelay.get() / 2;
            countTicks = placeDelay.get() - i;
            sHurt = true;
        }
    ...
    private void onPreSlowTick(TickEvent.Pre event) {
        ...
        if (bBreakerMain.get()
            ...
            && (!bBreakerOnlySur.get() || EntityInfo.isSurrounded(mc.player))
            ...
            switch (bBreakerMode.get()) {
                case Packet -> BedUtils.packetMine(burrowBp, bBreakerSwap.get(), breakTask);
                case Client -> BedUtils.normalMine(burrowBp, bBreakerSwap.get());
            }
            return;
        } else {
            stageTask.run(() -> {
            ...
            if (tBreakerMain.get()
                ...
                && (!tBreakerOnlySur.get() || EntityInfo.isSurrounded(mc.player))
                ...
                switch (tBreakerMode.get()) {
                    case Packet -> BedUtils.packetMine(trapBp, tBreakerSwap.get(), breakTask);
                    case Client -> BedUtils.normalMine(trapBp, tBreakerSwap.get());
                }
                return;
            } else {
                secondStageTask.run(() -> {
                ...
                if (sBreakerMain.get()
                    ...
                    && (!sBreakerOnlySur.get() || EntityInfo.isSurrounded(mc.player))
                    ...
                    for (CardinalDirection d : CardinalDirection.values()) {
                        ...
                        if (mc.world.getBlockState(cPos).getBlock().asItem().equals(Items.STRING) && mc.player.getPos().distanceTo(BlockInfo.getCenterVec3d(cPos)) < 4.5)
                            strings.add(cPos);

                        if (mc.world.getBlockState(cPos.offset(d.toDirection())).getBlock().asItem().equals(Items.STRING) && mc.player.getPos().distanceTo(BlockInfo.getCenterVec3d(cPos.offset(d.toDirection()))) < 4.5)
                    ...
                    if (!strings.isEmpty()) {
                        ...
                        for (BlockPos p : strings) {
                            ...
                            switch (sBreakerMode.get()) {
                                case Packet -> BedUtils.packetMine(p, false, breakTask);
                                default -> BedUtils.normalMine(p, false);
                            }
                        }
                    }
                }
    ...
    public void onBreakPacket(PacketEvent.Receive event) {
        ...
        if (event.packet instanceof BlockBreakingProgressS2CPacket) {
            ...
            Boolean = BlockInfo.getBlock(packetBp) instanceof BedBlock;
    ...
    private void doPlace() {
        ...
        BlockHitResult placeResult = new BlockHitResult(Vec3dInfo.closestVec3d(finalPos), Direction.UP, finalPos, false);
        BlockHitResult breakResult = new BlockHitResult(Vec3dInfo.closestVec3d(finalPos), Direction.UP, finalPos, false);
        ...
        double p = Rotations.getPitch(Vec3dInfo.closestVec3d(finalPos));
    ...
    private void calculateHolePos() {
        ...
        BlockPos p = EntityInfo.getBlockPos(target).up();

        double selfDMG = BedUtils.getDamage(mc.player, BlockInfo.getCenterVec3d(p), false, false, 0, true);
        double targetDMG = BedUtils.getDamage(target, BlockInfo.getCenterVec3d(p), false, false, 0, true);

        if (BedUtils.canBed(p.north(), p) && BlockInfo.isWithinRange(p.north(), placeRange.get()) && selfDMG < maxSelfDamage.get() && targetDMG > minTargetDamage.get()) {
            finalPos = p.north();
            placeDirection = CardinalDirection.South;
        } else if (BedUtils.canBed(p.south(), p) && BlockInfo.isWithinRange(p.south(), placeRange.get()) && selfDMG < maxSelfDamage.get() && targetDMG > minTargetDamage.get()) {
            finalPos = p.south();
            placeDirection = CardinalDirection.North;
        } else if (BedUtils.canBed(p.east(), p) && BlockInfo.isWithinRange(p.east(), placeRange.get()) && selfDMG < maxSelfDamage.get() && targetDMG > minTargetDamage.get()) {
            finalPos = p.east();
            placeDirection = CardinalDirection.West;
        } else if (BedUtils.canBed(p.west(), p) && BlockInfo.isWithinRange(p.west(), placeRange.get()) && selfDMG < maxSelfDamage.get() && targetDMG > minTargetDamage.get()) {
            finalPos = p.west();
    ...
    private void calculatePos() {
        ...
        ArrayList<BlockPos> sphere = new ArrayList<>(BedUtils.getTargetSphere(target, radius, 3));
        ...
        try {
            for (BlockPos p : sphere) {
                ...
                if (!Vec3dInfo.isWithinRange(BlockInfo.closestVec3d(p), placeRange.get()) || !BlockInfo.isReplaceable(p)) continue;

                //4 times loop for every direction
                for (CardinalDirection d : CardinalDirection.values()) {
                    double targetDMG = BedUtils.getDamage(target, BlockInfo.getCenterVec3d(p.offset(d.toDirection())), predict.get(), predictCollision.get(), predictIncrease.get(), ignoreTerrain.get());
                    double selfDMG = BedUtils.getDamage(mc.player, BlockInfo.getCenterVec3d(p.offset(d.toDirection())), predict.get(), predictCollision.get(), predictIncrease.get(), ignoreTerrain.get());
                    double friendDMG = 0;

                    if (antiFriendPop.get()) {
                        for (Entity entity : mc.world.getEntities()) {
                            if (entity instanceof PlayerEntity friend && Friends.get().isFriend(friend)) {
                                friendDMG = BedUtils.getDamage(friend, BlockInfo.getCenterVec3d(p.offset(d.toDirection())), false, false, 1, false);
                            }
                        }
                    }

                    if (!BedUtils.canBed(p, p.offset(d.toDirection()))
