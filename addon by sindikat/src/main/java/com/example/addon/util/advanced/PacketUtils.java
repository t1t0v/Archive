...
public class PacketUtils {
    ...
    public static void startPacketMine(BlockPos blockpos, boolean clientSwing) {
        start(blockpos);
        if (clientSwing) mc.player.swingHand(MAIN_HAND);
                else mc.getNetworkHandler().sendPacket(new HandSwingC2SPacket(MAIN_HAND));
        stop(blockpos);
    }
    ...
    public static void packetPlace(BlockPos pos, FindItemResult slot, boolean rotate, boolean clientSwing) {
        if (pos != null) {
            ...
            mc.getNetworkHandler().sendPacket(new PlayerInteractBlockC2SPacket(MAIN_HAND, result, 0));
